...
@Component
public class NaturalPersonAuthenticationBean {

	private static final Logger LOGGER = LogManager.getLogger(NaturalPersonAuthenticationBean.class);
	...
	@Autowired
	public NaturalPersonAuthenticationBean(MongoClient mongoClient) {
		this.mongoClient = mongoClient;
	}

	@Handler
	public void process(Exchange exchange, @Body NaturalPerson person) throws Exception {
		LOGGER.info("Authenticating natural person.");
		...
		if (foundPerson.getPersonType().equals(PersonType.LEGAL)) {
			throw new ObjectNotFoundException("User not found.");
		}
